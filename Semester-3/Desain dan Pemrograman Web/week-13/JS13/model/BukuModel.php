...
class BukuModel extends Model{
    ...
    public function insertData($data){
        if($this->driver == 'mysql'){
            ...
            // binding parameter ke query, "i" untuk integer, "s" untuk string, "sssi" berarti string-string-integer-string
            ...
            // eksekusi query untuk menyimpan ke database
            sqlsrv_query($this->db, "insert into {$this->table} (kategori_id, buku_kode, buku_nama, jumlah, deskripsi, gambar) values(?,?,?,?,?,?)", array($data['kategori_id'], $data['buku_kode'], $data['buku_nama'], $data['jumlah'], $data['deskripsi'], $data['gambar']));
        }
    }
    ...
    public function updateData($id, $data){
        if($this->driver == 'mysql'){
            ...
            $query->bind_param('iissssi', $data['kategori_id'], $data['buku_kode'], $data['buku_nama'], $data['jumlah'], $data['deskripsi'], $data['gambar'], $id);
            ...
            sqlsrv_query($this->db, "update {$this->table} set kategori_id = ?, buku_kode = ?, buku_nama = ?, jumlah = ?, deskripsi = ?, gambar = ? where buku_id = ?", [
                $data['kategori_id'], $data['buku_kode'], $data['buku_nama'], $data['jumlah'], $data['deskripsi'], $data['gambar'], $id
            ]);
        }
    }
